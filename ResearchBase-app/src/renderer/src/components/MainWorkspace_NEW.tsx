  ...
  const handleDrop = async (e: React.DragEvent<HTMLDivElement>): Promise<void> => {
    ...
    for (const file of pdfFiles) {
      try {
        ...
        if (!filepath) {
          console.error('Could not get file path for:', file.name)
          continue
        }

        console.log(`Ingesting: ${file.name}`)
        const result = await window.api.ingestDocument(filepath, file.name)

        if (result.success) {
          console.log('✅ Document ingested:', result.document)
              ...
              >
                ...
                <div className="text-center px-6">
                  ...
                  <p className="text-slate-400 text-sm">
                    Support for single or multiple research papers
                  </p>
                </div>
              </div>
            </div>
